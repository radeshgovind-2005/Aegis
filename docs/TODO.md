...
- [x] [phase-1/task-1.5] Add `dev:dummy-origin` and `dev:all` scripts to `package.json` so both Workers can be started with one command. ✓ `dev:dummy-origin` added (port 8788). `dev:all` skipped — two terminals is the correct UX for interactive wrangler dev sessions; no `concurrently` dep needed. (resolved 2026-04-23)
